const data = {
    "Sichuan": [
        {   
            name: "Kung Pao Chicken",
            description: "Kung pao chicken (宫保鸡丁gōngbǎo jīdīng /gong-baoww jee-ding/ 'palace-kept chicken cubes') is a traditional Sichuan dish made with chicken, chili, Sichuan peppers, peanuts, and vegetables. The chicken tastes tender and smooth, and the flavor can be either spicy or mild according to how much chili is added.",
            image: "sichuan/kungpaochicken.png"
        },
        {
            name: "test123",
            description: "",
            image: ""
        }
    ],
    "Beijing": [
        {
            name: "Peking Roast Duck",
            description: "filler",
            image: ""
        },
        {
            name: "test234",
            description: "",
            image: ""
        }
    ]
}

export {data};